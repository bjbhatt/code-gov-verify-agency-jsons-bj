...
class AgencyJsonStream extends Writable {
    ...
    verifyAgencyJson(agency, codeJson) {
        ...
        if (codeJson.releases) {
            ...
            codeJson.releases.forEach(function (release) {
                if (this.config.OPEN_SOURCE_USAGE_TYPES.indexOf(release.permissions.usageType) !== -1) {
                    verifyURLs.push({
                        "position": i,
                ...
                for (var validItem of validURLItems) {
                    if (validItem.repositoryURL && release.repositoryURL === validItem.repositoryURL) {
                        if (this.config.OPEN_SOURCE_USAGE_TYPES.indexOf(release.permissions.usageType) !== -1 && 
                            this.config.OPEN_SOURCE_USAGE_TYPES.indexOf(validItem.usageType) != -1) {
                            urlDuplicate = true;
                            urlDuplicates.push({
                                "position": i,
                                "organization": release.organization,
